...
class SpectralLibraryUtils:
    ...
    def createProfileField(
        ...
        if encoding == ProfileEncoding.Bytes:
            field = QgsField(name=name, type=QMetaType.Type.QByteArray, comment=comment)
        elif encoding == ProfileEncoding.Text:
            field = QgsField(name=name, type=QMetaType.Type.QString, len=-1, comment=comment)
        elif encoding == ProfileEncoding.Json:
            field = QgsField(name=name, type=QMetaType.Type.QVariantMap, typeName='JSON', comment=comment)
    ...
    def readFromVectorLayer(source: Union[str, QgsVectorLayer]) -> Optional[QgsVectorLayer]:
        ...
        for idx in range(source.fields().count()):
            ...
            if field.type() == QMetaType.Type.QByteArray and field.editorWidgetSetup().type() == '':
    ...
    def setAttributeMap(feature: QgsFeature, attributes: Dict):
        ...
        for f in feature.fields():
            ...
            if n in attributes:
                ...
                elif f.type() == QMetaType.Type.QDateTime:
                    if isinstance(v, datetime.datetime):
                        v = QDateTime.fromString(v.isoformat(), Qt.DateFormat.ISODate)
                    elif isinstance(v, datetime.date):
                        v = QDateTime.fromString(v.isoformat(), Qt.DateFormat.ISODate)
                elif f.type() == QMetaType.Type.QDate:
                    if isinstance(v, datetime.datetime):
                        v = QDateTime.fromString(v.isoformat(), Qt.DateFormat.ISODate)
                    elif isinstance(v, datetime.date):
                        v = QDateTime.fromString(v.isoformat(), Qt.DateFormat.ISODate)
    ...
    def createSpectralLibrary(
        ...
        with edit(lyr):
            ...
            if not (lyr.addAttribute(QgsField(name=FIELD_NAME, type=QMetaType.Type.QString))):
