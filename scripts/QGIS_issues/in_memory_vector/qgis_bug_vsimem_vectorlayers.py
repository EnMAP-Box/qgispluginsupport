...
if not (lyr1.isValid()):
    raise AssertionError
if not (lyr1.crs().isValid()):
    raise AssertionError
drv: ogr.Driver = ogr.GetDriverByName('GPKG')
ds2 = drv.CopyDataSource(ds1, path2)
if not (isinstance(ds2, ogr.DataSource)):
    raise AssertionError
lyr2: ogr.Layer = ds2.GetLayer(0)
if not (lyr2.GetSpatialRef().Validate() == ogr.OGRERR_NONE):
    raise AssertionError
del lyr2, ds2
