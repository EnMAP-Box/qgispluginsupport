...
class SpectralPropertyOrigin(object):
    ...
    GDALColorInterpretation = 'gdal_color_interpretation'
...
def anyValue(values: Optional[list]) -> bool:
    """
    Returns True if the input is a list with at least one non-None value.
    :param values:
    :return:
    """
    if values is None:
...
class QgsRasterLayerSpectralProperties(QgsObjectCustomProperties):
    ...
    def readFromGDALDataset(self, ds: gdal.Dataset, overwrite: bool = False):
        ...
        if band_wlu is None and band_wl is None:
            # try to use GDAL ColorInterpretation Flags
            _wlu = []
            _wl = []
            for b in range(ds.RasterCount):
                band: Band = ds.GetRasterBand(b + 1)
                ...
                if ci in GCI_WL_WLU:
                    wl1, wl2, wlu = GCI_WL_WLU[ci]
                    _wlu.append(wlu)
                    _wl.append(0.5 * (wl1 + wl2))
                else:
                    _wlu.append(None)
                    _wl.append(None)
            if any(_wl):
                o_wl = SpectralPropertyOrigin.GDALColorInterpretation
                o_wlu = SpectralPropertyOrigin.GDALColorInterpretation
                band_wl = _wl
                band_wlu = _wlu
...
class QgsRasterLayerSpectralPropertiesTable(QgsVectorLayer):
    ...
    def __init__(self):
        ...
        self.startEditing()
        bandNo = QgsField('band', type=QMetaType.Type.Int, comment='Band Number')
    ...
    def initDefaultFields(self):
        ...
        BBL = QgsField('BBL', type=QMetaType.Type.Bool, comment='Band Band List')
        BBL.setDefaultValueDefinition(QgsDefaultValue('True'))
        self.addAttribute(BBL)

        WL = QgsField('WL', type=QMetaType.Type.Double, comment='Wavelength of band center')
        self.addAttribute(WL)

        WLU = QgsField('WLU', type=QMetaType.Type.QString, comment='Wavelength Unit')
        ...
        WL_MIN = QgsField('WLmin', type=QMetaType.Type.Double, comment='Minimum Wavelength')
        self.addAttribute(WL_MIN)
        WL_MAX = QgsField('WLmax', type=QMetaType.Type.Double, comment='Maximum Wavelength')
        self.addAttribute(WL_MAX)

        FWHM = QgsField('FWHM', type=QMetaType.Type.Double, comment='Full width at half maximum')
